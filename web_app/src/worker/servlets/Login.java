...
public class Login extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(userNameFromSession == null){//user is not logged in
            String userNameFromReqParams = req.getParameter(Constant.USERNAME);
            int numOfThreads = Integer.parseInt(req.getParameter("numOfThreads"));
            ...
            if(userNameFromReqParams == null || userNameFromReqParams.isEmpty()){
                ...
                synchronized (this){
                    if(usersManager.isWorkerExist(userNameFromReqParams)){
                        String errorMessage = "Worker " + userNameFromReqParams + " already exists. Please enter a different worker name.";
                        resp.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                        resp.getOutputStream().print(errorMessage);
                    }else{
                        try{
                            usersManager.addWorker(userNameFromReqParams, numOfThreads);
                        }catch (Exception e)
                        {
                            e.printStackTrace();
                        }
